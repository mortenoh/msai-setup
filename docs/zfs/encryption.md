...
If you want **encryption of root itself**: on the canonical layout root is ext4 (see [Disk Partitioning](../ubuntu/installation/disk-partitioning.md#canonical-layout-ext4-root-two-independent-zfs-pools)), so root encryption is [**LUKS + LVM**](../ubuntu/installation/disk-partitioning.md#encrypted-alternative-zfs-native-encryption-or-luks-lvm) under GRUB — ZFS native encryption does not apply to an ext4 root. On this build ZFS native encryption is for the **data pools** (`hot`, `tank`, or a single dataset like `tank/secrets`). If instead you take the [ZFS Root alternative](../ubuntu/installation/zfs-root-alternative.md), root lives on ZFS and native encryption on that pool becomes the natural choice — set it at `zpool create` time and ZFSBootMenu prompts for the passphrase at boot. Either way, encryption never covers `/boot/efi` — the EFI partition is always unencrypted, same as any LUKS+GRUB setup.
