...
ZFS is the storage backbone of this build for **everything except the OS root**. Root itself lives on a small ext4 partition (Subiquity + GRUB — see [Disk Partitioning](../ubuntu/installation/disk-partitioning.md)); everything else that matters — Incus instance storage, databases, media, models, backups — lives on ZFS, split across **two independent pools, one per physical NVMe**: `hot` on the fast 4 TB drive (slot 1, PCIe 4.0 x4) and `tank` on the slow 2 TB drive (slot 2, PCIe 4.0 x1). No pool is striped across both drives. Root-on-ZFS with boot environments is a [documented alternative](../ubuntu/installation/zfs-root-alternative.md), not the default.
...
!!! info "ZFS is the canonical filesystem for this build"
    ZFS is the **production, source-of-truth** filesystem here — Incus instance datasets, databases, media, models, and backups all live on it, and nothing else in this build stores real data (the OS root is a small, rebuildable ext4 partition). The [Bcachefs](../bcachefs/index.md) section is documented purely as an experimental/exploratory alternative to learn from; it is **not** used anywhere on the MS-S1 MAX.
...
hot  (fast 4 TB NVMe, slot 1, PCIe 4.0 x4 — hot data + Incus instances)
  +-- /dev/disk/by-id/nvme-...-part4   (the primary's ~3.4 TB left after EFI + /boot + ext4 root)
...
1. **Two independent single-disk pools, no redundancy, no striping across drives.** With only two unequal-speed NVMe devices on the MS-S1 MAX, each pool is one disk. We rely on snapshots + replication (see [Backup & Recovery](../operations/backup.md)) instead of RAID. If you lose a drive, you restore that pool from a remote replica. Two separate pools (rather than one striped across both) mean anything on `hot` is guaranteed on the fast drive — see [Pool Creation → Two pools, not one stripe](pool-creation.md#two-pools-not-one-stripe). The [VirtualBox Lab](virtualbox-lab.md) shows how to wire up mirrored and raidz setups so you can practice them even though the real install isn't using them.
2. **Root is deliberately NOT on ZFS.** The OS lives on a small ext4 partition (Subiquity + GRUB); it's thin and rebuildable, so it forgoes snapshot rollback while containers, VMs, and data — everything that matters — keep full ZFS snapshot coverage. Root-on-ZFS with ZFSBootMenu boot environments is a [documented alternative](../ubuntu/installation/zfs-root-alternative.md), not the default.
3. **Instance storage is Incus's, not hand-carved.** Containers and VMs are ZFS datasets under `hot/incus`, created and owned by Incus automatically — there is no manual `tank/containers/<svc>` or `tank/vm` tree anymore. See [Incus storage](../incus/storage.md).
...
| [VM Storage](vm-storage.md) | How Incus backs VM disks with ZFS zvols under `hot/incus`; volblocksize/recordsize background; cross-refs to [Incus storage](../incus/storage.md). |
